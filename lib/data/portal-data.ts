...
export async function getDashboardData(organizationId: string) {
  ...
  const [modulesRes, obligationsRes, controlsRes, actionsRes, openActionsRes, evidenceRes] = await Promise.all([
    ...
    supabase.from("action_items").select("id,title,status,priority,due_date").eq("organization_id", organizationId).neq("status", "done").order("due_date", { ascending: true }).limit(8),
    supabase.from("action_items").select("id", { count: "exact", head: true }).eq("organization_id", organizationId).neq("status", "done"),
    supabase.from("evidence").select("id", { count: "exact", head: true }).eq("organization_id", organizationId),
  ]);

  const error = modulesRes.error || obligationsRes.error || controlsRes.error || actionsRes.error || openActionsRes.error || evidenceRes.error;
  ...
  return {
    metrics: {
      ...
      technical: pct(compliantControls, activeControls.length),
      obligations: activeObligations.length,
      pending,
      inProgress,
      controls: activeControls.length,
...
export async function getActionsData(organizationId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from("action_items").select("id,title,description,source_type,status,priority,due_date").eq("organization_id", organizationId).order("due_date", { ascending: true });
  if (error) throw new Error(error.message);
  return data ?? [];
}
...
export async function getDecisionsData(organizationId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from("product_decisions").select("id,code,priority,category,question,recommendation,status,answer,answered_at").eq("organization_id", organizationId).order("code");
  if (error) throw new Error(error.message);
  return data ?? [];
}
...
export async function searchOrganizationData(organizationId: string, query: string) {
  ...
  const error = obligationsRes.error || controlsRes.error || actionsRes.error;
  if (error) throw new Error(error.message);
  return { obligations: obligationsRes.data ?? [], controls: controlsRes.data ?? [], actions: actionsRes.data ?? [] };
}
...
export async function getAdministrationData() {
  ...
  const error = organizationsRes.error || grantsRes.error;
  if (error) throw new Error(error.message);
  return { organizations: organizationsRes.data ?? [], grants: grantsRes.data ?? [] };
}
