...
export function DataTable({
  ...
  const filterOptions = useMemo(() => {
    if (!filterKey) return [];
    return Array.from(new Set(rows.map((row) => text(row[filterKey])).filter((value) => value !== "—"))).sort();
  }, [rows, filterKey]);
  ...
  return (
    <div className="smart-table">
      <div className="table-toolbar">
        ...
        {filterKey ? (
          <label className="table-filter"><span>{filterLabel}</span><select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">Todos</option>{filterOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>
        ) : null}
        <details className="column-menu">
          ...
          <div className="column-popover">
            ...
            {columns.map((column) => (
              <label key={column.key}><input type="checkbox" checked={visible.includes(column.key)} onChange={(event) => {
      ...
      <div className="table-wrap">
        <table className="table">
          <thead><tr>{activeColumns.map((column) => <th key={column.key}>{column.sortable === false ? column.label : <button type="button" className="th-sort" onClick={() => sortBy(column.key)}>{column.label}{sortKey === column.key ? (sortDirection === "asc" ? <ArrowDownAZ size={14} /> : <ArrowUpAZ size={14} />) : null}</button>}</th>)}</tr></thead>
          <tbody>
            {filtered.length ? filtered.map((row, index) => (
              <tr key={String(row.id || index)}>
                {activeColumns.map((column) => {
                  ...
                  if (column.kind === "multiline") return <td key={column.key}><div className="table-title">{text(value)}</div>{column.secondaryKey && row[column.secondaryKey] ? <div className="table-sub">{text(row[column.secondaryKey])}</div> : null}</td>;
